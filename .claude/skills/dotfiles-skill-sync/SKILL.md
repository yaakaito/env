...
description: Clone 済みのこの環境リポジトリのルートで実行し、setup.sh と dotfiles/ を基準に端末上の dotfiles、Claude/Codex skills、関連 CLI 設定を安全に同期・更新・整理する。Use when the user asks to set up, reconcile, migrate, update, clean, prune, or merge this repository's dotfiles or skills with an existing terminal, especially when existing local files must be inspected before applying repository versions.
---

# Dotfiles Skill Sync

この skill は、このリポジトリを clone した作業ツリーのルートで使う。`setup.sh`、`dotfiles/`、`skills/` を基準に、端末固有の変更を壊さずに dotfiles と skills を同期する。端末へ配る skill は repo root の `skills/` にあり、`gh skill` で管理する。
...
2. 作業ツリーを確認する。
   - `git status --short`
   - ユーザーの未コミット変更を上書きしない。
3. `setup.sh` と `dotfiles/` の現状を読む。

## Read First

必要な作業に応じて reference を読む。

- `references/repository-map.md`: `setup.sh` が管理するコピー先、skill 配置、インストール対象を確認する。
...
1. リポジトリ状態を確認する。
   ...
   - `setup.sh`、`dotfiles/`、`skills/` の現在内容
2. 端末側の対象ファイルを棚卸しする。
   - `~/.gitconfig`
   - `~/.config/git/ignore`
   - `~/.claude`
   ...
   - `~/.zsh/peco.zsh`, `~/.zsh/git-worktree.zsh`, `~/.zsh/vscode-extensions.zsh`, `~/.zsh/bin/git-worktree-add`
3. 各差分を分類する。
   ...
   - **Remove candidate**: `setup.sh` や現行 dotfiles から外れ、古くなった可能性があるファイルや設定。
4. 同期計画を短く提示する。
   - 取り込むもの、ローカルに残すもの、マージするもの、削除候補を分ける。
   - 削除候補は必ずユーザー確認を得る。
   - 秘匿情報の可能性がある差分は内容をそのまま表示しすぎず、キー名や構造だけで確認する。
5. 合意済みの変更だけ実施する。
   - repo に入れる変更は `dotfiles/`、`setup.sh`、または関連 skill に反映する。
   - 端末へ反映する変更はバックアップまたは一時退避を作ってから適用する。
   - `.zshrc` への追記は重複しないように検査してから行う。
6. 検証する。
   - `git diff --check`
   - 変更した Markdown / shell / TOML / JSON の構文確認
   - 必要なら `zsh -n`、`jq`、`git config --get` などの軽量検証
