...
- `setup.yaml` が参照するファイルと repo の実体が食い違う（`./setup.sh --check` が失敗する）。
...
```bash
git log --oneline --decorate -- setup.yaml setup.sh dotfiles skills .claude/skills/dotfiles-skill-sync
git log --oneline --decorate -- skills dotfiles/.codex .claude/skills
git log --name-status -- setup.yaml setup.sh dotfiles skills .claude/skills
```

端末へ配る skill は過去 `dotfiles/.claude/skills` に置かれていた。repo root `skills/` への移動より前の履歴は旧 path か `git log --follow` で辿る。同期対象の定義も過去は `setup.sh` に直接書かれていた（`setup.yaml` 導入前）。

特定ファイルの過去内容を見る。

```bash
git show <commit>:setup.yaml
...
```bash
git diff <old>..<new> -- setup.yaml setup.sh dotfiles skills .claude/skills
```
