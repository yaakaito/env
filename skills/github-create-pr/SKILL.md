...
1 つの PR に収めるとレビューしにくい変更は、依存関係のある小さな PR の連なり(stack)に分ける選択肢がある。次のどちらかに当てはまる場合は、PR を作る前にユーザーへ stack にするかを確認する。

- diff が大きい。目安として変更が数百行を超える、または多数のファイルにまたがる。
- 変更が複数の文脈に分けられる。たとえば、データモデルの追加と API の追加と画面の変更が同じブランチに乗っていて、それぞれ独立してレビューできる。

確認するときは、分割案を具体的に示す。各 branch に載せる関心と commit を挙げ、単一 PR のままにする選択肢も並べる。分割の根拠が薄いのに機械的に聞かない。`gh stack` が使えない環境では確認せず、単一 PR で進める。

- stack を選んだ場合は `gh-stack` skill の手順に従って stack を構築し、`gh stack submit` で PR 群を作る。各 PR の本文は、以降の手順を PR ごとに適用して書く。
- 単一 PR を選んだ場合、または該当しない場合は、そのまま続ける。

この確認は、次の手順の意図に関する質問とまとめて 1 回で聞いてよい。

### 6. 変更の意図が分からない場合は質問する
